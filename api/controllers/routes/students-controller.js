...
router.put('/students/:id', upload.any(), function (req, res, next) {
    ...
    if (updateStudent.specialNeeds) {
        updateStudent.specialNeeds = updateStudent.specialNeeds.split(',');
    }
    ...
    studentService.find({ _id: req.params.id, _createdBy: req.user._id })
        .then((data) => {
            console.log('Student Edit Found', data);
            console.log('Student Edit Trocar por', updateStudent);
